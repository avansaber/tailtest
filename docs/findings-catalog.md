...
`/tailtest:gen` produces a starter test file, not a finding. The output file has two special lines at the top:

**Line 1 -- mandatory review header:**
```
# generated by tailtest, review before committing
```

**Line 2 -- detection note:**
```
# tailtest detected: InvoiceStatus, CreditLimitExceeded -- review before committing
```

The detection note tells you what domain tailtest inferred before generating. There are four variants:

| Variant | When it fires |
|---|---|
| `# tailtest detected: <entities>` | AST signals found in the source file (enums, exceptions, key types) |
| `# tailtest context: <summary>` | No AST signals; project has been deep-scanned (`/tailtest scan --deep`) |
| `# tailtest used your description: <text>` | User passed `--context "..."` flag |
| `# tailtest: no domain context available -- review generated tests carefully` | No context available; generated tests are generic |

**What to do with the detection note:**
- If it looks right, proceed to review the tests.
- If it looks wrong (wrong domain, missing entities), re-run with `--context "description of what this module does"` to override.
- Run `/tailtest scan --deep` once on the project to enable the project-summary context for all future generation.

**Token budget:** The generator fits its context into ~7,500 characters. On very large source files, optional sections (test style sample, domain signals, category hints) are stripped in that order. The source file contents are never truncated.
