...
@pytest.mark.asyncio
async def test_generate_tests_rejects_context_over_300_chars(tmp_path: Path) -> None:
    """context argument exceeding 300 chars must return an error response."""
    tool = GenerateTestsTool(tmp_path)
    response = await tool.invoke({
        "file": "src/widget.py",
    ...
    assert response["isError"] is True
    assert "300" in response["content"][0]["text"]


@pytest.mark.asyncio
async def test_generate_tests_context_threaded_to_detection_note(tmp_path: Path) -> None:
    """user_context from the 'context' arg appears in line 2 of the generated file."""
    from unittest.mock import patch

    src = tmp_path / "src" / "billing.py"
    src.parent.mkdir(parents=True)
    src.write_text("def charge(amount: int) -> None:\n    pass\n")

    generated = (
        "# generated by tailtest, review before committing\n"
        "def test_charge():\n"
        "    assert charge(10) is None\n"
    )
    outer = {"type": "result", "result": generated, "total_cost_usd": 0.001}
    ...
    with (
        ...
        response = await tool.invoke({
            "file": str(src),
            "project_root": str(tmp_path),
            "context": "invoice approval engine",
        })

    assert response["isError"] is False
    payload = json.loads(response["content"][0]["text"])
    assert payload["status"] == "ok"
    content = Path(payload["test_path"]).read_text()
    lines = content.splitlines()
    assert "invoice approval engine" in lines[1]
